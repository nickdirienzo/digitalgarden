---
{"dg-publish":true,"permalink":"/what-if-nano-claw-skills-didn-t-need-to-change-the-codebase/","tags":["essay"],"created":"2026-02-28T09:34:02.709-08:00","updated":"2026-03-01T02:21:47.407-08:00"}
---
...
Inside the container, a scoping proxy connects to the bridge and enforces per-group authorization. Each group declares which tools it can access and which parameters are pinned: a family group chat can only `send_message` to its own JID. The agent never sees tools it isn't allowed to use.

The inbound entrypoint script we originally designed? Gone. The manifest replaced it entirely:
...
Pure config. The MCP server handles the complexity of auth, pagination, transport, and state internally. That's its job, not ours.
